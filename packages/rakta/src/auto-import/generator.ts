...
function buildImportLines(
    ...
    return exports
        .map((discoveredExport) => `export * from "${discoveredExport.importPath}";`)
        .join("\n");
};
...
export function generateAutoImports(
    ...
    writeFileSync(
        join(outputDir, "auto-imports.ts"),
        tsContent,
        "utf-8"
    );
    ...
    if (options.generateDts) {
        ...
        writeFileSync(
            join(outputDir, "auto-imports.d.ts"),
            dtsContent,
            "utf-8"
        );
    }
...
export function printAutoImportSummary(
    manifest: AutoImportManifest
): void {
    console.log("\n  Rakta Auto Import");
