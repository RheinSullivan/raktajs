...
describe("package statistics parsing", () => {
	test("counts runtime dependencies from the latest npm version only", () => {
		const parsed = parseRuntimeDependencies({
			...
			time: { "1.1.5": "2026-08-12T00:00:00.000Z" },
			versions: {
				"1.1.4": {
		...
		expect(parsed.dependencies).toBe(2);
		expect(parsed.dependencyNames).toEqual(["gsap", "react-icons"]);
		expect(parsed.version).toBe("1.1.5");
		expect(parsed.updatedAt).toBe("2026-08-12T00:00:00.000Z");
	});
