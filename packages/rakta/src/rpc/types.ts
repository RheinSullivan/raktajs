...
export type RouterDefinition = Record<
	string,
	ProcedureDefinition<unknown, unknown>
>;

/** Extracts the inferred input type of a procedure. */
export type InferInput<TProcedure> =
	TProcedure extends ProcedureDefinition<infer TInput, unknown>
		? TInput
		: never;

/** Extracts the inferred output type of a procedure. */
export type InferOutput<TProcedure> =
	TProcedure extends ProcedureDefinition<unknown, infer TOutput>
		? TOutput
		: never;
