...
module Generators
  ...
  def self.create_project(name, mode = 'spa')
    ...
    target_dir = File.expand_path(name, Dir.pwd)
    ...
    File.write(File.join(target_dir, 'package.json'), JSON.pretty_generate(pkg))
    ...
    File.write(File.join(target_dir, 'rakta.config.ts'), rakta_config)
    ...
    File.write(File.join(target_dir, 'app', 'page.tsx'), app_page)
    puts "✓ Project scaffolded successfully in #{target_dir}"
  end
end
...
if __FILE__ == $PROGRAM_NAME
  name = ARGV[0] || 'my-app'
  mode = ARGV[1] || 'spa'
  Generators.create_project(name, mode)
end
